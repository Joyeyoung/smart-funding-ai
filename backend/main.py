...
def label_to_korean(label):
    ...
    return info["ko_name"] if info and "ko_name" in info else label.replace('_', ' ')

def rgb_to_color_name(rgb):
    r, g, b = rgb
    if max(rgb) - min(rgb) < 20:
        return "하얀색" if np.mean(rgb) > 200 else "검은색" if np.mean(rgb) < 50 else "회색"
...
def analyze_design(image_bytes):
    ...
    return {
        "main_color": rgb_to_color_name(avg_rgb),
        "material": guess_material(avg_rgb)
    }
